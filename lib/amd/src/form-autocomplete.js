...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification'], function($, log, str, templates, notification) {
    ...
    var addNavigation = function(options, state, originalSelect) {
        ...
        // Prevent suggestions auto-closing when scrolling list (MDL-63198).
        suggestionsElement.parent().on('mousedown', '[role=option], [role=listbox]', function() {
            var pendingKey = 'form-autocomplete-mousedown';
            M.util.js_pending(pendingKey);
            window.setTimeout(function() {
                inputElement.focus();
            }, 500);
