...
$PAGE->set_url('/user/editmahara.php', array('course'=>$course, 'id'=>$userid));
...
if ($course->id != SITEID) {
    ...
    if (empty($SESSION->wantsurl)) {
        $SESSION->wantsurl = $CFG->httpswwwroot.'/user/editmahara.php';
    }
...
if ($usernew = $userform->get_data()) {
    ...
    if ($CFG->emailchangeconfirmation) {
        ...
        if (isset($usernew->email) and $user->email != $usernew->email && !has_capability('moodle/user:update', $systemcontext)) {
            ...
            $email_changed_html .= $OUTPUT->continue_button("$CFG->wwwroot/auth/mnet/jump.php?hostid=4&wantsurl=artefact/internal/");
            $email_changed = true;
        }
    }
    ...
    if (!$email_changed || !$CFG->emailchangeconfirmation) {
        redirect("$CFG->wwwroot/auth/mnet/jump.php?hostid=4&wantsurl=artefact/internal/");
    }
}
